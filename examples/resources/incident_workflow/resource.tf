...
# via form fields when they run it. The collected values are then available in
# the workflow scope for use by conditions and steps.
resource "incident_workflow" "page_execs" {
  ...
  form_fields = [
    {
      name        = "Reason for paging"
      type        = "Text"
      description = "Why are we paging the execs?"
      placeholder = "e.g. Major customer-facing outage"
      array       = false
      required    = true
    },
  ]
