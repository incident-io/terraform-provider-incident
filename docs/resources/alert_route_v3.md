...
page_title: "incident_alert_route_v3 Resource - terraform-provider-incident"
subcategory: ""
description: |-
  ...
  This resource uses the v3 alert routes API, which configures grouping via a dedicated grouping_config, combines channels and message templates into message_config, and nests the incident template under incident_config. For the previous API, see incident_alert_route.
...
# incident_alert_route_v3 (Resource)
...
- `alert_sources` (Attributes Set) Which alert sources this route matches (see [below for nested schema](#nestedatt--alert_sources))
...
- `enabled` (Boolean) Whether this alert route is enabled
...
- `is_private` (Boolean) Whether this alert route is private. Private alert routes only create private incidents from alerts.
- `message_config` (Attributes) (see [below for nested schema](#nestedatt--message_config))
- `name` (String) The name of this alert route, for the user's reference
...
- `id` (String) Unique identifier for this alert route
...
- `group_keys` (Attributes Set) Which attributes should this alert route use to group alerts? (see [below for nested schema](#nestedatt--grouping_config--default--group_keys))
- `window_seconds` (Number) How long should the grouping window be?
- `window_type` (String) How the grouping window behaves: 'rolling' extends end_at as new alerts attach, 'fixed' closes at a set time. Possible values are: `rolling`, `fixed`.
...
- `message_template` (Attributes) (see [below for nested schema](#nestedatt--message_config--message_template))
...
<a id="nestedatt--message_config--message_template"></a>
### Nested Schema for `message_config.message_template`

Optional:

- `array_value` (Attributes List) The array of literal or reference parameter values (see [below for nested schema](#nestedatt--message_config--message_template--array_value))
- `value` (Attributes) The literal or reference parameter value (see [below for nested schema](#nestedatt--message_config--message_template--value))

<a id="nestedatt--message_config--message_template--array_value"></a>
### Nested Schema for `message_config.message_template.array_value`
...
<a id="nestedatt--message_config--message_template--value"></a>
### Nested Schema for `message_config.message_template.value`
...
terraform import incident_alert_route_v3.example 01ABC123DEF456GHI789JKL
```
