...
- `selected_rota_id` (String) For a schedule target with a rota-scoped `schedule_mode`, the rota to page. A bound target can leave this unset and bind to an expression that navigates the schedule's `rotations` and filters them by name instead.
