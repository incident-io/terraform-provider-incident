...
- **Provider produced inconsistent result** - Often issues with `Computed` attributes. Also watch out for API responses that return nested objects even when the user didn't configure them (e.g. `EmailOptions` always returned for email sources because it contains `email_address`). The `FromAPI` method should return nil when no user-configurable data is present. **Set-typed nested collections are especially fragile**: the framework correlates set elements by (raw) value and does NOT apply semantic equality inside them, so any read-back divergence from the planned element breaks correlation with "does not correlate with any element in actual". When hunting one of these, check the *core API serializer*, not just the provider — the API may echo fields the user never set. Example (ONC-12476): `incident_alert_route` escalation-path targets have their binding duplicated into `users` by `server/api/serialize/alert_route_escalation_bindings.go` (legacy frontend compat, pending ONC-5335); the fix was to prefer `escalation_paths` and drop the duplicate `users` in the v2/v3 `FromAPI`. A target is always either escalation-path OR user, never both (enforced by `EscalationBinding.Validate` in core).
