...
- **JSON-ish string fields (e.g. engine `literal`)** - Use the `jsontypes.NormalizedString` custom type (`internal/provider/jsontypes`) for schema fields that hold JSON, or arbitrary literals that may be JSON. It implements semantic equality: when both old and new values parse as JSON they are compared by canonical form (key-sorted, escaping-insensitive so `>` and `>` are equal); otherwise it falls back to exact string equality, so plain references/strings still work. This fixes "Provider produced inconsistent result after apply" and perpetual diffs caused by different JSON encoders (HCL `jsonencode` HTML-escapes, but CDKTF `JSON.stringify`, `file()` and heredocs do not). Apply it via `CustomType: jsontypes.NormalizedStringType{}` on the schema attribute, the matching model field, and every `types.StringType` object-attribute-type entry for that field (mismatches panic at runtime). DO NOT try to fix escaping mismatches by toggling `SetEscapeHTML` — escaping is normalised on only one side, so any fixed choice just moves the breakage between the jsonencode and raw-string user populations (this was round-tripped in ONC-7057 / ONC-7504). Semantic equality is the correct fix.