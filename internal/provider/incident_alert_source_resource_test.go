...
func TestAlertSourceResourceSchema(t *testing.T) {
	...
	r := NewIncidentAlertSourceResource()
...
// TestAlertSourceFromAPIDisabled covers pausing a heartbeat. The API only returns
// disabled for source types that support it, so a null on any other type must stay null.
func TestAlertSourceFromAPIDisabled(t *testing.T) {
	t.Run("reads a paused heartbeat", func(t *testing.T) {
		source := alertSourceV3("heartbeat")
		source.Disabled = lo.ToPtr(true)

		if !fromAPI(t, source, &alertSourceModel{}).Disabled.ValueBool() {
	...
	t.Run("reads a monitoring heartbeat", func(t *testing.T) {
		...
		got := fromAPI(t, source, &alertSourceModel{})
	...
	t.Run("keeps the configured value when the API omits it", func(t *testing.T) {
		config := &alertSourceModel{Disabled: types.BoolValue(false)}
	...
	t.Run("never stores an unknown", func(t *testing.T) {
		config := &alertSourceModel{Disabled: types.BoolUnknown()}
	...
	t.Run("stays null for a source type that cannot pause", func(t *testing.T) {
		got := fromAPI(t, alertSourceV3("http"), &alertSourceModel{}).Disabled
		if !got.IsNull() {
