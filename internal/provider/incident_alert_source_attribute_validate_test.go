...
func alertSourceAttributeSchemaType(t *testing.T) (tfsdk.Config, tftypes.Object) {
	...
	NewIncidentAlertSourceAttributeResource().Schema(context.Background(), resource.SchemaRequest{}, &schemaResp)
...
func validateAlertSourceAttribute(t *testing.T, config tfsdk.Config) diag.Diagnostics {
	...
	r, ok := NewIncidentAlertSourceAttributeResource().(*alertSourceAttributeResource)
	if !ok {
		t.Fatalf("NewIncidentAlertSourceAttributeResource did not return a *alertSourceAttributeResource")
	}
