...
func TestIncidentEngineParamBindingValue_JSONOrdering(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "html_characters_preserved",
			apiJSON:            `{"type":"doc","content":[{"type":"paragraph","content":[{"type":"varSpec","attrs":{"name":"alert.title","label":"Alert -> Title","missing":false}}]}]}`,
			expectedNormalized: `{"content":[{"content":[{"attrs":{"label":"Alert -> Title","missing":false,"name":"alert.title"},"type":"varSpec"}],"type":"paragraph"}],"type":"doc"}`,
			description:        "HTML characters like > should be preserved, not escaped",
		},
	}
