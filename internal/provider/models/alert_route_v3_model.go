...
func (AlertRouteV3ResourceModel) FromAPIWithPlan(apiModel client.AlertRouteV3, plan *AlertRouteV3ResourceModel) AlertRouteV3ResourceModel {
	...
	// Grouping config.
	result.GroupingConfig = &AlertRouteV3GroupingConfigModel{
		Default: &AlertRouteV3GroupingSettingsModel{
			...
			WindowSeconds: types.Int64Value(int64(apiModel.GroupingConfig.Default.WindowSeconds)),
			WindowType:    types.StringValue(string(apiModel.GroupingConfig.Default.WindowType)),
		},
	}
	for _, gk := range apiModel.GroupingConfig.Default.GroupKeys {
		result.GroupingConfig.Default.GroupKeys = append(result.GroupingConfig.Default.GroupKeys, AlertRouteGroupingKey{
	...
	result.IncidentConfig = &AlertRouteV3IncidentConfigModel{
		...
		ConditionGroups:    conditionGroupsFromV3(apiModel.IncidentConfig.ConditionGroups),
...
func (m AlertRouteV3ResourceModel) ToCreatePayload() client.AlertRoutesCreatePayloadV3 {
	...
	if m.GroupingConfig != nil && m.GroupingConfig.Default != nil {
		...
		payload.GroupingConfig = client.AlertGroupingConfigV3{
			Default: client.GroupingSettingsV3{
				Enabled:       m.GroupingConfig.Default.Enabled.ValueBool(),
				GroupKeys:     groupKeys,
				WindowSeconds: int32(m.GroupingConfig.Default.WindowSeconds.ValueInt64()),
				WindowType:    client.GroupingSettingsV3WindowType(m.GroupingConfig.Default.WindowType.ValueString()),
			},
		}
	}
		...
		for _, destination := range m.MessageConfig.Destinations {
			...
			if destination.SlackTargets != nil && destination.SlackTargets.Binding != nil {
				payloadDestination.SlackTargets = &client.AlertRouteChannelTargetPayloadV3{
					ChannelVisibility: destination.SlackTargets.ChannelVisibility.ValueString(),
			...
			if destination.MsTeamsTargets != nil && destination.MsTeamsTargets.Binding != nil {
				payloadDestination.MsTeamsTargets = &client.AlertRouteChannelTargetPayloadV3{
					ChannelVisibility: destination.MsTeamsTargets.ChannelVisibility.ValueString(),
	...
	if m.IncidentConfig != nil {
		payload.IncidentConfig = client.AlertRouteIncidentConfigPayloadV3{
			...
			ConditionGroups:    conditionGroupsToV3Payload(m.IncidentConfig.ConditionGroups),
