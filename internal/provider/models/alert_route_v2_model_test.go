...
// A route with incident creation disabled still carries an incident_template.name block,
// and the API answers it with no binding, so the read has nothing to build one from. It
// answers summary either way: omitted, or present with no binding of its own.
//
// Assert on the binding's type rather than its nullness: both spellings are null, and only
// the typed one can be written to state. Regression test for #597.
func TestAlertRouteV2TypesAnUnboundIncidentTemplate(t *testing.T) {
	ctx := context.Background()

	for name, summary := range map[string]*client.AlertRouteAutoGeneratedTemplateBindingV2{
		"summary omitted":         nil,
		"summary with no binding": {Autogenerated: false},
	} {
		t.Run(name, func(t *testing.T) {
			planName := NullAutoGeneratedParamBinding(types.BoolValue(false))
			planSummary := NullAutoGeneratedParamBinding(types.BoolValue(false))
			plan := AlertRouteResourceModel{
				IncidentTemplate: &AlertRouteIncidentTemplateModel{
					Name:    &planName,
					Summary: &planSummary,
				},
			}
			...
			api := client.AlertRouteV2{
				...
				IncidentTemplate: client.AlertRouteIncidentTemplateV2{
					Name:    client.AlertRouteAutoGeneratedTemplateBindingV2{Autogenerated: false},
					Summary: summary,
				},
			}

			model := AlertRouteResourceModel{}.FromAPIV2WithPlan(api, &plan)

			for field, binding := range map[string]*AlertRouteAutoGeneratedParamBinding{
				"name":    model.IncidentTemplate.Name,
				"summary": model.IncidentTemplate.Summary,
			} {
				t.Run(field, func(t *testing.T) {
					require.NotNil(t, binding)
					assertTypedNullBinding(t, ctx, *binding)
				})
			}
		})
	}
...
// assertTypedNullBinding asserts a binding is both unset and writable to state. Nullness
// alone passes against #597: an untyped null is null too, and the framework rejects it.
func assertTypedNullBinding(t *testing.T, ctx context.Context, binding AlertRouteAutoGeneratedParamBinding) {
	t.Helper()

	wantValue := types.ObjectType{AttrTypes: ParamBindingValueAttrTypes()}
	assert.True(t, binding.Value.IsNull(), "value: nothing is bound")
	assert.True(t, wantValue.Equal(binding.Value.Type(ctx)),
		"value: expected %s, got %s", wantValue, binding.Value.Type(ctx))

	wantArray := ParamBindingValueListType()
	assert.True(t, binding.ArrayValue.IsNull(), "array_value: nothing is bound")
	assert.True(t, wantArray.Equal(binding.ArrayValue.Type(ctx)),
		"array_value: expected %s, got %s", wantArray, binding.ArrayValue.Type(ctx))
}
