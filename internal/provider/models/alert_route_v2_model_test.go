...
// and the API answers it with no binding, so the read has nothing to build one from.
//
...
func TestAlertRouteV2TypesAnUnboundIncidentTemplate(t *testing.T) {
	...
	// The API omits summary entirely, which is the third branch of the same read.
	api := client.AlertRouteV2{
		Id:              "01ABC",
	...
	for name, binding := range map[string]*AlertRouteAutoGeneratedParamBinding{
		...
		t.Run(name, func(t *testing.T) {
			...
			assert.True(t, binding.Value.IsNull(), "nothing is bound")
			assert.True(t, wanted.Equal(binding.Value.Type(ctx)),
				"value: expected %s, got %s", wanted, binding.Value.Type(ctx))

			assert.True(t, binding.ArrayValue.IsNull(), "nothing is bound")
			assert.True(t, ParamBindingValueListType().Equal(binding.ArrayValue.Type(ctx)),
				"array_value: expected %s, got %s",
				ParamBindingValueListType(), binding.ArrayValue.Type(ctx))
		})
	}
}
