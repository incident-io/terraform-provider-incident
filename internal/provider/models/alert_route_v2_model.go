...
type AlertRouteResourceModel struct {
	...
	// v3-only top-level blocks. The presence of grouping_config switches the
	// resource into v3 mode (see isV3Mode): grouping moves into its own
	// grouping_config, channel_config + message_template collapse into
	// message_config, and the incident template moves under
	// incident_config.template. In v2 mode these are nil.
...
// IsV3Mode reports whether this configuration targets the v3 alert routes API.
// The presence of the top-level grouping_config block is the discriminator: it
// is required in v3 and absent in v2.
func (m AlertRouteResourceModel) IsV3Mode() bool {
	return m.GroupingConfig != nil
}
...
type AlertRouteIncidentConfigModel struct {
	...
	AutoRelateGroupedAlerts types.Bool                    `tfsdk:"auto_relate_grouped_alerts"`
	ConditionGroups         IncidentEngineConditionGroups `tfsdk:"condition_groups"`
...
// AlertRouteAutoGeneratedParamBinding is a binding the API may fill in itself, so it carries
// an autogenerated flag alongside the two long forms. It holds them as framework types for
// the same reason IncidentEngineParamBinding does: a config may point either at something
// Terraform hasn't settled yet.
...
// NullAutoGeneratedParamBinding is a binding with nothing bound, for the same reason
// NullParamBinding exists: the zero value of its list and object carry no type, and the
// framework can't write those to state.
func NullAutoGeneratedParamBinding(autogenerated types.Bool) AlertRouteAutoGeneratedParamBinding {
	return AlertRouteAutoGeneratedParamBinding{
...
func (b AlertRouteAutoGeneratedParamBinding) ToParamBinding() IncidentEngineParamBinding {
	...
	binding.ArrayValue = listOrNull(b.ArrayValue, ParamBindingValueType())
	binding.Value = objectOrNull(b.Value, ParamBindingValueAttrTypes())
...
func (AlertRouteAutoGeneratedParamBinding) FromParamBinding(binding IncidentEngineParamBinding, autogenerated types.Bool) AlertRouteAutoGeneratedParamBinding {
	return AlertRouteAutoGeneratedParamBinding{
		Autogenerated: autogenerated,
		ArrayValue:    nullIfEmptyList(binding.ArrayValue, ParamBindingValueType()),
...
func (AlertRouteAutoGeneratedParamBinding) FromParamBindingWithPlan(binding IncidentEngineParamBinding, autogenerated types.Bool, planBinding *AlertRouteAutoGeneratedParamBinding) AlertRouteAutoGeneratedParamBinding {
	...
	// A plan that wrote `array_value = []` keeps its empty list rather than reading back as
	// unset, which would fail the apply as an inconsistent result.
	if result.ArrayValue.IsNull() && planBinding != nil {
		planned := planBinding.ArrayValue
		if !planned.IsNull() && !planned.IsUnknown() && len(planned.Elements()) == 0 {
			result.ArrayValue = planned
		}
	}
...
// SeverityObjectNull returns a null severity object with the correct attribute types.
func SeverityObjectNull() types.Object {
...
func SeverityToPayload(ctx context.Context, severityObj types.Object) *client.AlertRouteSeverityBindingPayloadV2 {
	if severityObj.IsNull() || severityObj.IsUnknown() {
		return nil
	}

	var severity AlertRouteSeverityModel
	severityObj.As(ctx, &severity, basetypes.ObjectAsOptions{})

	payload := &client.AlertRouteSeverityBindingPayloadV2{
		MergeStrategy: client.AlertRouteSeverityBindingPayloadV2MergeStrategy(severity.MergeStrategy.ValueString()),
	}

	if severity.Binding != nil {
		bindingPayload := severity.Binding.ToPayload()
...
func (AlertRouteResourceModel) FromAPIV2(apiModel client.AlertRouteV2) AlertRouteResourceModel {
...
func (AlertRouteResourceModel) FromAPIV2WithPlan(apiModel client.AlertRouteV2, plan *AlertRouteResourceModel) AlertRouteResourceModel {
	...
	nameBinding := NullAutoGeneratedParamBinding(types.BoolValue(apiModel.IncidentTemplate.Name.Autogenerated))
	...
	summaryBinding := NullAutoGeneratedParamBinding(types.BoolValue(false))
	...
	// Only set custom_fields if API returns them or plan has them
	// Otherwise leave as nil to preserve null state in Terraform
...
func (m AlertRouteResourceModel) ToCreatePayloadV2() client.AlertRoutesCreatePayloadV2 {
	...
	if m.IncidentTemplate != nil {
		...
		incidentTemplate.Severity = SeverityToPayload(context.Background(), m.IncidentTemplate.Severity)
...
func (m AlertRouteResourceModel) ToUpdatePayloadV2() client.AlertRoutesUpdatePayloadV2 {
	createPayload := m.ToCreatePayloadV2()
