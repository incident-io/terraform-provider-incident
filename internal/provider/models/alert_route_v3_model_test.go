...
func apiArrayBindingV3(value client.EngineParamBindingValueV3) *client.EngineParamBindingV3 {
	return &client.EngineParamBindingV3{
		ArrayValue: &[]client.EngineParamBindingValueV3{value},
	}
}

func alertRouteV3WithTargets(targets []client.AlertRouteEscalationTargetV3) client.AlertRouteV3 {
...
// TestAlertRouteV3EscalationTargetUsersEcho mirrors the v2 echo coverage
// through the v3 conversion path (issue #390): the API echoes a target's
// escalation_paths binding in users, which must not enter state unless the
// plan configured users on that target.
func TestAlertRouteV3EscalationTargetUsersEcho(t *testing.T) {
	pathLiteralV3 := client.EngineParamBindingValueV3{Literal: lo.ToPtr("01PATH")}
	pathLiteralV2 := client.EngineParamBindingValueV2{Literal: lo.ToPtr("01PATH")}
	userLiteralV3 := client.EngineParamBindingValueV3{Literal: lo.ToPtr("01USER")}
	userLiteralV2 := client.EngineParamBindingValueV2{Literal: lo.ToPtr("01USER")}

	t.Run("echoed users dropped", func(t *testing.T) {
		api := alertRouteV3WithTargets([]client.AlertRouteEscalationTargetV3{
			{EscalationPaths: apiArrayBindingV3(pathLiteralV3), Users: apiArrayBindingV3(pathLiteralV3)},
		})
		plan := planWithEscalationTargets([]AlertRouteEscalationTargetModel{
			{EscalationPaths: planArrayBinding(pathLiteralV2)},
		})

		model := AlertRouteResourceModel{}.FromAPIV3WithPlan(api, plan)

		targets := model.EscalationConfig.EscalationTargets
		if len(targets) != 1 {
			t.Fatalf("targets: got %d, want 1", len(targets))
		}
		if targets[0].Users != nil {
			t.Error("echoed users should be dropped")
		}
		if targets[0].EscalationPaths == nil {
			t.Error("escalation_paths should be preserved")
		}
	})

	t.Run("users-only and dual-field targets preserved", func(t *testing.T) {
		api := alertRouteV3WithTargets([]client.AlertRouteEscalationTargetV3{
			{Users: apiArrayBindingV3(userLiteralV3)},
			{EscalationPaths: apiArrayBindingV3(pathLiteralV3), Users: apiArrayBindingV3(userLiteralV3)},
		})
		plan := planWithEscalationTargets([]AlertRouteEscalationTargetModel{
			{Users: planArrayBinding(userLiteralV2)},
			{EscalationPaths: planArrayBinding(pathLiteralV2), Users: planArrayBinding(userLiteralV2)},
		})
		...
		if targets[0].Users == nil {
			t.Error("users-only target should keep users")
		}
		if targets[1].Users == nil {
			t.Error("configured users on dual-field target should be kept")
		}
	})
}
