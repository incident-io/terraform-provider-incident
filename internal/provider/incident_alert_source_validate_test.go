...
func alertSourceSchemaType(t *testing.T) (tfsdk.Config, tftypes.Object) {
	...
	NewIncidentAlertSourceResource().Schema(context.Background(), resource.SchemaRequest{}, &schemaResp)
...
func validateAlertSource(t *testing.T, config tfsdk.Config) diag.Diagnostics {
	...
	r, ok := NewIncidentAlertSourceResource().(*alertSourceResource)
	if !ok {
		t.Fatalf("NewIncidentAlertSourceResource did not return a *alertSourceResource")
	}
...
// TestAlertSourceValidateDisabled covers pausing: only heartbeat sources support it,
// and sending disabled on any other type is a 422 at apply.
func TestAlertSourceValidateDisabled(t *testing.T) {
	diags := validateAlertSource(t, alertSourceConfig(t, map[string]tftypes.Value{
		"source_type": stringValue("http"),
