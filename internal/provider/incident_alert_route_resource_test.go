...
// TestAccIncidentAlertRouteResourceBranchesValidation tests that expressions using branches
// operations with a non-"." root_reference are rejected at plan time.
func TestAccIncidentAlertRouteResourceBranchesValidation(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
...
// TestAccIncidentAlertRouteResource_OwningTeamIDs checks that owning_team_ids work correctly for alert routes.
func TestAccIncidentAlertRouteResourceOwningTeamIDs(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
...
func TestAccIncidentAlertRouteResourceRawJSONLiteral(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
...
// TestAccIncidentAlertRouteResourceEscapedJSONLiteral proves the HCL jsonencode
...
func TestAccIncidentAlertRouteResourceEscapedJSONLiteral(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
...
// TestAccIncidentAlertRouteResourceCastExpression covers the `cast` operation on
...
func TestAccIncidentAlertRouteResourceCastExpression(t *testing.T) {
...
// TestAccIncidentAlertRouteResourceEmptyOwningTeamIDs is the v2-schema half of
// TestAccIncidentAlertRouteV3ResourceEmptyOwningTeamIDs: the null-vs-empty mapping is
// duplicated per schema version (FromAPIV2WithPlan and FromAPIV3WithPlan), so a route
// created with an explicitly empty owning_team_ids is checked on both paths.
func TestAccIncidentAlertRouteResourceEmptyOwningTeamIDs(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
...
func testAccIncidentAlertRouteResourceConfigOwningTeams(teamCount int, setEmpty bool) string {
	...
	return testRunTemplate("incident_alert_route_v2_owning_teams", `
data "incident_catalog_type" "team" {
...
resource "incident_catalog_entry" "owner_team_{{ $i }}" {
  catalog_type_id  = data.incident_catalog_type.team.id
  external_id      = {{ stableSuffix (printf "tf-alert-route-v2-empty-owning-team-%d" $i) | quote }}
  name             = {{ stableSuffix (printf "tmv2-%d" $i) | quote }}
  attribute_values = []
}
{{ end }}
...
// A v2 route must supply incident_template even when it creates no incidents: the
// v2 API requires it, and only the v3 schema rejects a template alongside
// incident_config.enabled = false. So a page-only route carries the template below,
// bound to nothing, and the API has no binding to return.
//
// Refreshing that route has failed since v6.12.0 (#597), which is what the refresh
// step catches: an unbound binding has to survive the round trip and leave the plan
// empty.
//
// There's no import step: ImportState probes the v3 API first, so a migrated
// organisation imports any route into the v3 schema and ImportStateVerify can never
// match a v2 configuration.
func TestAccIncidentAlertRouteResourceIncidentCreationDisabled(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccIncidentAlertRouteResourceConfigIncidentCreationDisabled("page-only"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("incident_alert_route.page_only", "incident_config.enabled", "false"),
					resource.TestCheckResourceAttr("incident_alert_route.page_only", "incident_template.name.autogenerated", "false"),
					resource.TestCheckResourceAttr("incident_alert_route.page_only", "incident_template.summary.autogenerated", "false"),
					resource.TestCheckNoResourceAttr("incident_alert_route.page_only", "incident_template.name.value"),
					resource.TestCheckNoResourceAttr("incident_alert_route.page_only", "incident_template.summary.value"),
				),
			},
			{
				RefreshState: true,
...
func testAccIncidentAlertRouteResourceConfigIncidentCreationDisabled(name string) string {
	name = StableSuffix(name)

	return fmt.Sprintf(`
resource "incident_alert_route" "page_only" {
  name       = %[1]q
  enabled    = true
  is_private = false
