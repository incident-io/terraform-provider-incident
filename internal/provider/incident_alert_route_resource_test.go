...
// TestAccIncidentAlertRouteResourceMessageTemplate tests that message_template works correctly for alert routes.
func TestAccIncidentAlertRouteResourceMessageTemplate(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			// Create alert route with message_template
			{
				Config: testAccIncidentAlertRouteResourceConfigWithMessageTemplate("test-route-with-message-template"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("incident_alert_route.test", "name", "test-route-with-message-template"),
					resource.TestCheckResourceAttrSet("incident_alert_route.test", "message_template.value.reference"),
				),
			},
			// ImportState testing
			{
...
func testAccIncidentAlertRouteResourceConfigWithMessageTemplate(name string) string {
	return fmt.Sprintf(`
# Look up the Message Templates catalog type
data "incident_catalog_type" "message_templates" {
  name = "Message Templates"
}

# Create a message template catalog entry for this test
resource "incident_catalog_entry" "test_message_template" {
  catalog_type_id = data.incident_catalog_type.message_templates.id
  external_id     = "tf-alert-route-message-template-test"
  name            = "Terraform Alert Route Message Template Test"
  attribute_values = []
}

resource "incident_alert_source" "test" {
  name        = "Test Source for Message Template"
  source_type = "http"
  template = {
    title = {
      literal = jsonencode({
...
resource "incident_alert_route" "test" {
  ...
  message_template = {
    value = {
      reference = "catalog_entry[\"${incident_catalog_entry.test_message_template.id}\"]"
    }
  }
}
`, name)
