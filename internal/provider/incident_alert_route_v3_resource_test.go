...
// TestAccIncidentAlertRouteV3ResourceIncidentTemplateOmitted covers issue #486.
// `incident_config.template` is optional, but the API returns a fully-populated
// default template whenever incident creation is enabled, so creating a route
// that omits it used to fail the apply with "produced an unexpected new value:
// .incident_config.template: was null, but now cty.ObjectVal(...)" and leave the
// resource marked for replacement on every subsequent run.
func TestAccIncidentAlertRouteV3ResourceIncidentTemplateOmitted(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccIncidentAlertRouteV3ResourceConfigWithTemplate("test-route-v3-no-template", ""),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("incident_alert_route.test", "incident_config.enabled", "true"),
					// The API's default template must not leak into state.
					resource.TestCheckNoResourceAttr("incident_alert_route.test", "incident_config.template.name.autogenerated"),
					resource.TestCheckNoResourceAttr("incident_alert_route.test", "incident_config.template.severity.merge_strategy"),
				),
			},
			// Refresh and ensure there's no perpetual diff.
			{
...
func testAccIncidentAlertRouteV3ResourceConfig(name string) string {
	return testAccIncidentAlertRouteV3ResourceConfigWithTemplate(name, alertRouteV3IncidentTemplateBlock)
}

// testAccIncidentAlertRouteV3ResourceConfigWithTemplate is
// testAccIncidentAlertRouteV3ResourceConfig with the incident template spliced
// in, so callers can vary it. Pass "" to omit the optional template entirely.
func testAccIncidentAlertRouteV3ResourceConfigWithTemplate(name, incidentTemplate string) string {
