...
// The v3 schema rejects a template alongside incident_config.enabled = false, so the
// shape behind #597 can't be reached here (see
// TestAccIncidentAlertRouteV3ResourceIncidentTemplateValidation). An autogenerated
// name and summary are the v3 route to the same unbound binding: they carry no value
// of their own.
func TestAccIncidentAlertRouteV3ResourceAutoGeneratedTemplate(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccIncidentAlertRouteV3ResourceConfigAutoGeneratedTemplate("v3-autogen"),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("incident_alert_route.autogen", "incident_config.template.name.autogenerated", "true"),
					resource.TestCheckResourceAttr("incident_alert_route.autogen", "incident_config.template.summary.autogenerated", "true"),
					resource.TestCheckNoResourceAttr("incident_alert_route.autogen", "incident_config.template.name.value"),
					resource.TestCheckNoResourceAttr("incident_alert_route.autogen", "incident_config.template.summary.value"),
				),
			},
			{
				RefreshState: true,
			...
			{
				ResourceName:      "incident_alert_route.autogen",
				ImportState:       true,
				ImportStateVerify: true,
			},
		},
...
func testAccIncidentAlertRouteV3ResourceConfigAutoGeneratedTemplate(name string) string {
	name = StableSuffix(name)

	return fmt.Sprintf(`
resource "incident_alert_route" "autogen" {
  name       = %[1]q
  enabled    = true
  is_private = false
