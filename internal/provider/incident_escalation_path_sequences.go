...
type sequenceCodec[P any, N any] interface {
	...
	LeafPayload(ctx context.Context, id string, node escalationPathNode, diags *diag.Diagnostics) (P, bool)
	...
	Leaf(ctx context.Context, node N, diags *diag.Diagnostics) (escalationPathNode, bool)
}

// pathSequenceCodec is the codec for an escalation path's own nodes.
type pathSequenceCodec struct{}

func (pathSequenceCodec) LeafPayload(ctx context.Context, id string, node escalationPathNode, diags *diag.Diagnostics) (client.EscalationPathNodePayloadV2, bool) {
...
func (pathSequenceCodec) Leaf(ctx context.Context, node client.EscalationPathNodeV2, diags *diag.Diagnostics) (escalationPathNode, bool) {
	converted := escalationPathNode{}
...
func unflattenSequencesWith[P any, N any](ctx context.Context, codec sequenceCodec[P, N], start string, sequences map[string][]escalationPathNode, diags *diag.Diagnostics) []P {
...
func unflattenSequence[P any, N any](ctx context.Context, codec sequenceCodec[P, N], key string, sequences map[string][]escalationPathNode, visiting, inlined map[string]bool, diags *diag.Diagnostics) []P {
...
func flattenSequencesWith[P any, N any](ctx context.Context, codec sequenceCodec[P, N], nodes []N, prior escalationPathPriorNames, diags *diag.Diagnostics) (string, map[string][]escalationPathNode) {
	sequences := map[string][]escalationPathNode{}
...
func flattenSequence[P any, N any](ctx context.Context, codec sequenceCodec[P, N], nodes []N, priorKey, fallbackKey string, prior escalationPathPriorNames, sequences map[string][]escalationPathNode, diags *diag.Diagnostics) string {
	...
	for index, node := range nodes {
		...
		var converted escalationPathNode
		if conditions, thenPath, elsePath, ok := codec.Branch(node); ok {
			...
			branch := &escalationPathBranch{
				If:   escalationPathBranchIfFromAPI(ctx, conditions, diags),
...
func chooseSequenceKey(nodeIDs []string, priorKey, fallbackKey string, taken map[string][]escalationPathNode) string {
	if priorKey != "" {
...
func escalationPathSequencesToMap(ctx context.Context, sequences map[string][]escalationPathNode, diags *diag.Diagnostics) types.Map {
	return sequencesToMap(ctx, escalationPathNodeAttrTypes(), sequences, diags)
}
...
func sequencesToMap(ctx context.Context, nodeAttrTypes map[string]attr.Type, sequences map[string][]escalationPathNode, diags *diag.Diagnostics) types.Map {
