...
- Check that every resource v7 renames takes a `moved` block from the name it went by, by walking the resources the provider registers rather than a list kept by hand: a promotion that forgets its state mover, or leaves a `_beta` name registered, now fails the build rather than someone's upgrade. A `moved` block against a resource with no mover fails the plan outright, and only whoever already wrote the block ever sees it.
- **Breaking**: `incident_alert_source_beta` and `incident_alert_source_attribute_beta` are now `incident_alert_source` and `incident_alert_source_attribute`, and the `incident_alert_source` that declared a source and every attribute it populated together, under one `template.attributes` list, is removed. A source now holds only its own configuration, with `title`, `description`, `is_private` and `visible_to_teams` at the top level where `template` used to be, and each attribute binding is its own resource: filling in one more attribute is an add rather than a rewrite of the list, and two people editing different attributes no longer edit the same resource. Both promoted resources take a `moved` block from the name they went by. Coming off the old schema is a rewrite plus an import per resource - one source becomes a source and an attribute apiece - which the [migration guide](https://registry.terraform.io/providers/incident-io/incident/latest/docs/guides/migrating-to-v7) covers, including the `<alert_source_id>:<alert_attribute_id>` IDs the bindings import by. The `incident_alert_source` and `incident_alert_sources` data sources are unchanged, so the plural one still reports a source's bindings as a `template`, which is the shape the API returns rather than the shape the resources now take.
- **Breaking**: `incident_escalation_path_beta` is now `incident_escalation_path`, and the resource that nested a branch's nodes under `if_else.then_path` and `if_else.else_path` is removed, along with its data source. An escalation path is written as a flat map of named sequences from here on: a `branch` names the sequences to continue down instead of holding their nodes, `loop` replaces `repeat`, and `branch.if` replaces the raw engine `conditions` with one attribute per thing an escalation can be tested on. The five-level limit on branching goes with the nesting, so a path too deep to write before can be written now. The promoted resource takes a `moved` block from `incident_escalation_path_beta`, so anyone already on it renames and applies a plan that reports moves and nothing else. Coming off the old shape is a rewrite, but the escalation path itself is untouched: import it by ID rather than recreating it and everything escalating through it carries on, as the [migration guide](https://registry.terraform.io/providers/incident-io/incident/latest/docs/guides/migrating-to-v7) sets out.
- **Breaking**: `incident_schedule_beta` and `incident_schedule_rotation_beta` are now `incident_schedule` and `incident_schedule_rotation`, and the `incident_schedule` that declared its rotations inline is removed, along with its data source. A schedule and its rotations are separate resources from here on, so a change to one rotation no longer rewrites the whole schedule - which is what used to disturb who was on call elsewhere on it, or drop overrides attached to a shift. Both promoted resources take a `moved` block from the name they went by, so anyone already on the beta resources renames them and applies a plan that reports moves and nothing else. Coming off the old schema is not a rename, because one resource becomes a schedule plus a rotation apiece: do that on v6, where both schemas are registered, then upgrade - the [migration guide](https://registry.terraform.io/providers/incident-io/incident/latest/docs/guides/migrating-to-v7) has the field mapping and the import IDs. `handover_start_at` is now the rotation's `first_interval_starts_at`, and a rotation's `layers` are gone: a rotation with two layers is one rotation with `concurrent_shifts = 2`. `users = []` is rejected where the old schema accepted it - a rotation with nobody in it is `users = ["NOBODY"]`. `incident_schedule_replica`, `incident_schedule_sync_rule` and `incident_schedule_sync_target` are unchanged, though a replica source still names a layer, which the new rotation resource has no way to give you: replicating a Terraform-managed schedule needs a layer ID from elsewhere until that is addressed.
- Add a [Migrating to v7](https://registry.terraform.io/providers/incident-io/incident/latest/docs/guides/migrating-to-v7) guide, which is what v7 needs to be a version people can get to. It covers both moves: from the beta resources to the names they keep in v7, which is a rename and a `moved` block per resource, and from the v6 schemas to the ones that replace them, which is a rewrite plus an import because one `incident_schedule` becomes a schedule and a rotation apiece, and one `incident_alert_source` becomes a source and an attribute apiece. The schema work is written to be done on v6, where both schemas exist side by side: upgrading first leaves state under a reused resource name that v7 cannot read, and the guide says how to recover from that too. It also spells out that the old resource must be forgotten with `destroy = false` rather than destroyed, since both resources point at the same object and destroying one deletes the schedule and everyone's shifts.
