...
- Fix creating an `incident_alert_route` that enables incident creation but omits the optional `incident_config.template`. The API returns a fully-populated default template whenever incident creation is enabled, even when none was sent, so the provider wrote that default into state and the apply failed with "produced an unexpected new value: .incident_config.template: was null, but now cty.ObjectVal(...)" - leaving the route created in incident.io but marked for replacement on every subsequent run. The template now stays null when your config omits it, and is still read from the API on import.
