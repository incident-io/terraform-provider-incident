...
- Add `fixed_team_id` to `incident_alert_source` and `incident_alert_source_beta`, which fixes the team every alert from the source is attributed to instead of deriving it per alert from the payload. Set `fixed_team_id = incident_team.platform.id` and the organisation's team alert attribute is managed from that value; remove the attribute to un-fix the source and make the team bindable again. The team has to exist and your organisation needs a team alert attribute configured. On `incident_alert_source_beta` both are checked at plan time, and while the field is set an `incident_alert_source_attribute_beta` resource binding the team attribute is rejected when you apply. On `incident_alert_source` neither is checked until you apply, because the v2 validate endpoint doesn't take the field, and you should not also bind the team attribute under `template.attributes` — the API ignores a team binding sent in the template and leaves it out of reads, so a config carrying both never settles.
