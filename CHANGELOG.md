...
- Add the `incident_escalation_path_template` resource, which manages an escalation path template: an escalation path with parameters, from which many escalation paths can be built. A template is written the same way as `incident_escalation_path`, as a flat map of named node sequences, and declares `params`. A level or notify_channel target may carry a `binding` to one of them instead of an `id` - `binding = { value_reference = "primary_schedule" }` pages whatever schedule a path binds that param to - or to one of the template's `expressions` through `expression_ref`. Editing a template changes every path built from it.
- `incident_escalation_path_template` no longer offers `default_value` on a parameter. incident.io treats a template's parameters as holes each path fills, so a default was accepted and then dropped, and every apply that set one failed with "Provider produced inconsistent result after apply". An optional parameter a path leaves unbound skips the levels that use it rather than falling back to a value.
- `incident_escalation_path_template` now claims the template it manages, the way every other managed resource does, so the dashboard shows it as synced, refuses to edit it, and offers to disconnect one you want to hand back. The claim happens on create, on update and on import, the last subject to `mark_imported_resources_as_managed`. It needs a version of incident.io that knows `escalation_path_template` as a managed-resource type; against an older one the claim is rejected and the apply reports it.
- A template with no `params` is rejected when you plan rather than part way through an apply. The API requires at least one, since a template that binds nothing is just an escalation path.
- `incident_escalation_path_template` checks the escalation paths built from it at plan time. A template is edited in one place and read by many paths, so a plan now reports any path the edit would leave degraded or broken, naming the path and what would stop paging - a parameter the path never bound, a target that no longer resolves. The check runs on an edit rather than a create, since a template with no paths has nothing to check, and it reports rather than blocks: a template can legitimately be edited ahead of the paths that use it. A template the API rejects outright still fails the plan.
- `incident_escalation_path` can build a path from a template. Set `kind = "templated"` and `template_id`, and bind each of the template's params in `param_bindings`, keyed by the param's name, in place of `start` and `sequences`: a templated path takes its nodes, working hours and repeat config from the template, so those four attributes are rejected on one. `kind` mirrors the API rather than being inferred from whether `template_id` is set, so the two are validated against each other: `template_id` or `param_bindings` without `kind = "templated"` is rejected when you plan, as is `kind = "templated"` without a `template_id`. Leaving `kind` out means standalone. A path is one kind for life, so changing `kind` replaces it, while switching from one template to another keeps the kind and is an in-place update. The `incident_escalation_path` data source reports `kind`, `template_id` and `param_bindings` too.
...
- **Breaking**: the `incident_alert_source` data source now returns a source in the same shape as the resource, so `template` is gone and `title`, `description`, `priority`, `visible_to_teams`, `is_private` and `named_expression` take its place. It also takes `name` as well as `id`.
- The Go module path is now `github.com/incident-io/terraform-provider-incident/v7`.
- **Breaking**: the plural data sources are removed. `incident_alert_sources`, `incident_catalog_entries`, `incident_incident_types` and `incident_schedule_replicas` are gone; the singular `incident_alert_source`, `incident_catalog_entry` and `incident_schedule_replica` remain, and `incident_incident_type` is new. A data source that returns a list is a different kind of thing from one that returns an object: you cannot reference an element of it without indexing into an order the API decides, so a configuration built on one breaks when somebody adds a catalog entry or an alert source. The singular ones take the lookup you actually have - an ID, or a name - and fail loudly when it matches nothing or more than one thing, rather than silently resolving to whatever came back first. Note that the `incident_catalog_entries` **resource** is unaffected: managing a type's entries as one object is exactly what it is for, and only the data source of that name is removed. What you lose with them is enumeration. `incident_alert_sources` could filter by `source_type`, `incident_catalog_entries` could list a type's entries, and `incident_schedule_replicas` was the only way to discover a replica's ID - the singular takes an ID you have to know already, so if you were using the plural one to find IDs for import, get them from the dashboard or the API instead.
- Add the `incident_incident_type` data source, which looks up a single incident type by `id` or by `name`, replacing `incident_incident_types`. Incident types can't be created from Terraform - they're configured in your settings - so this is how you get hold of one's ID, for an alert route or a workflow that only applies to incidents of one type. It reports `description`, `create_in_triage`, `is_default`, `private_incidents_only` and `owning_team_ids`. Set exactly one of the two lookup attributes; setting both, or neither, is rejected at plan time. Incident type names aren't unique as far as the API is concerned, so a name matching more than one type is reported rather than resolved arbitrarily.
- Add a [Migrating to v7](https://registry.terraform.io/providers/incident-io/incident/latest/docs/guides/migrating-to-v7) guide, which puts in one place what each resource's page says about its own upgrade. It covers both paths. A configuration on the `_beta` resources needs nothing to upgrade: the names still work, and renaming each one is a `moved` block the provider carries the state across, which can be done whenever. A configuration on the v6 schemas keeps its state — those names now mean the schemas that replaced them, and the attributes that are gone are dropped as the state is read — but needs its configuration rewritten, because one `incident_schedule` is now a schedule and a rotation apiece and one `incident_alert_source` is a source and an attribute apiece; the guide gives the before and after for each, the IDs to import the split-out resources by, and the two changes to look for in the first plan afterwards, being the sequence names an escalation path reads back and the `ack_mode` default that moved from `all` to `first`. It also covers doing the schema work on v6.13 first, where both schemas exist side by side, for anyone who would rather verify it before upgrading.
- Cover the v6-to-v7 upgrade with acceptance tests, for the half of the migration nothing exercised: a configuration that never used the `_beta` resources. The rename has state movers behind it and tests to match; upgrading straight from the v6 schemas has neither, and could not have a mover - the v6 resources kept their names, so `incident_schedule` in v7 reads state that v6's `incident_schedule` wrote, and what makes that work is Terraform dropping the attributes the new schema no longer has as it decodes prior state. That is behaviour of Terraform rather than of this provider, which is exactly why it needed pinning down: it is the load-bearing claim of the migration guide, it is invisible in our own code, and nothing here would have noticed it breaking until somebody's on-call schedule was recreated. Each test applies the old configuration with the released v6.13.0 provider from the registry, which is the only way to get state in the old shape now the schema that wrote it is gone, then applies the rewritten configuration against the build under test and checks the object's ID is the same one - an upgrade that recreated the schedule would set an ID too, just not that one. The escalation path test reproduces both of the surprises the guide warns about, so the advice it gives is what the test asserts: the sequence names the refresh derives, and a level's `ack_mode` written out to keep the behaviour v6's default gave it.
...
- A panic while building a request is now reported as an error against the attribute that caused it, rather than crashing the provider. `incident_catalog_entry`, `incident_catalog_entries` and `incident_catalog_type_attribute` panicked when an attribute value wasn't the type the schema said it was, and when converting one produced diagnostics they had no way to return. Terraform reports a panic as a plugin crash, carrying a Go stack trace that says nothing about which resource or attribute was at fault, and it takes down the rest of the apply with it: resources that would have applied fine never get the chance. These paths now return diagnostics naming the attribute and, for a value inside an array, its index - so the failure lands on the resource that caused it, reads as an error you can act on, and the other resources in the graph carry on. `golangci-lint` now forbids `panic` outside tests, so a new one can't reach the request path unnoticed; the assertions in `internal/apischema` stay exempt, because they run while the provider's own schemas are built, before any request, and a wrong name there fails identically on every run.
- `incident_schedule_beta`, `incident_escalation_path_beta` and `incident_alert_source_beta` now take a `moved` block from the resource each of them replaces, so moving onto them is a change you can review in a pull request rather than a `terraform state rm` and a fresh import run by hand against production state. Both resources either side of a move manage the same object through the same API, so what the move carries is that object's ID, along with the attributes the new resource keeps from prior state rather than reading back - a schedule's `team_ids`, an alert source's `owning_team_ids` and `filter_condition_groups`. Everything the new schema says differently is left for the refresh Terraform runs straight after a move, which reads it from the API in the shape the new schema wants: an escalation path's nodes. An alert source's `title` and `description` move as well, out of the `template` the v6 resource holds them in, so the document you wrote survives the move rather than coming back in the API's own spelling of the same thing. Nothing is destroyed, so no on-call history or alert routing is disturbed. Two things to know before you write one. A `moved` block has a single target, so the resources that split out of one of these - a schedule's rotations, an alert source's attribute bindings - are imported alongside the move rather than moved, by `<schedule id>:<rotation id>` and `<alert source id>:<alert attribute id>`. And an escalation path's sequence names are not stored by the API, so the refresh names them: the sequence a path starts with is `main`, and the sequences a branch leads to are `main_then` and `main_else`. A configuration that calls them something else plans a change, so plan after the move and before you apply. One change to look for in that plan: `incident_escalation_path_beta` defaults a level's `ack_mode` to `first` where `incident_escalation_path` defaults it to `all`, and a default applies wherever the configuration is silent, so a level that never set `ack_mode` plans a move from `all` to `first` - after which the first person to acknowledge cancels everyone else's escalation on that level. Write `ack_mode = "all"` on every level whose behaviour you mean to keep. Each resource's page documents the move it accepts.
- Syntax-highlight the Terraform snippets in resource descriptions, which the registry rendered as plain text, and remove the stray non-breaking spaces that made comments in the `incident_alert_source` example render as `##&nbsp;`.
- Add `force_destroy` to `incident_maintenance_window`, which lets a destroy end a window that is still active. Without it such a destroy fails, and with it the window ends immediately without running its `resolve_on_end` and `reroute_on_end` actions.
...
- Fix the `incident_status` example, which created a status with `category = "closed"`. Closed statuses cannot be created through the API, so the example as documented did not work. It now shows two active statuses ordered by rank.
- Reject a secret name longer than 1024 bytes at plan time. The API caps it there, so an oversized name used to fail part way through an apply; the plan now says so instead. The limit is on bytes rather than characters, so a name using multi-byte characters reaches it sooner.
- Document using a secret in a workflow: the `incident_secret` page now has a second example wiring one into a `webhook.send` step, both ways the step takes a secret. A header written as `{{ <secret id> }}` through `data.incident_rich_text` with the `plain_single_line_with_secrets` feature set has its value substituted when the workflow runs, and the step's signing secret takes the secret's `id` directly to sign the request. Neither passes the value through Terraform.
- Add the `incident_api_key` resource, which manages an API key: the credential an integration authenticates to incident.io with, and the roles that decide what it may do. A key's token is returned when incident.io issues one - on create, and on each rotation - and at no other time, so it is stored in state as the sensitive `token` attribute rather than being re-read: anything that can read your state can read the token, so pass it on through a sensitive output rather than copying it around. Rotating is asked for by changing `token_version`, conventionally by incrementing it, the same way `value_wo_version` rotates a secret; the previous token keeps working for `rotation_grace_period_minutes`, which defaults to 30, so whatever holds it has a window to pick up the new one. Editing a key's name or roles never rotates it, so a key whose scopes you have just narrowed carries on working until you rotate it deliberately. A key imported with `terraform import` has no token, because the one it was issued with cannot be read back - rotating it is the only way to get one Terraform can hand on. incident.io will not let a key grant more than its own bearer holds, and refuses to assign `api_keys_manage` at all, so which roles you can assign depends on the key running Terraform rather than on anything the configuration can see: the plan asks incident.io whether the key it describes would be accepted, and a role you cannot grant is reported when you plan rather than part way through an apply. If that check cannot be reached the plan warns and carries on, so the rejection may still arrive at apply time.
- Add the `incident_api_key` data source, which looks up an existing API key by `id` or by `name`, so you can read the roles held by a key the dashboard owns without managing it as a resource. It reports `role_names`, `team_ids`, `team_role_names`, `comments` and the timestamps, including `last_used_at`, which is null for a key nothing has ever authenticated with. There is no token attribute: a lookup can say what a key may do, but never how to authenticate as it. Set exactly one of the two lookup attributes; setting both, or neither, is rejected at plan time. API key names are not unique, so a name matching more than one key is reported rather than resolved arbitrarily.
- Add the `incident_schedule_replicas` data source, which lists the replicas for an existing schedule by `schedule_id`. Use this to find replica IDs for import, or to read replica configuration the dashboard owns. To look up one replica you already have an ID for, keep using `incident_schedule_replica`.
- Add the `incident_schedule_sync_target` data source, which looks up an existing schedule sync target (a Slack user group that schedules can sync to) by `id` or by `slack_user_group_id`. It reports `add_bot_to_group`, `slack_team_id`, and the `linked_schedules` with an active rule pointing at it. Set exactly one of the two lookup attributes; setting both, or neither, is rejected at plan time.
- Add the `incident_schedule_sync_rule` data source, which looks up an existing schedule sync rule by `schedule_id` and `id`. Rules are nested under a schedule in the API, so both IDs are required, the same way they are for import. It reports `schedule_sync_target_id`, `sync_type`, `rotation_id`, and `permanent_member_user_ids`.
- Add the `incident_secret` resource, which manages a secret: a named credential a workflow can reference, such as the auth token for an outgoing webhook. A secret's value is write-only - incident.io stores it encrypted and never returns it - so it is set through `value_wo`, a [write-only attribute](https://developer.hashicorp.com/terraform/language/resources/ephemeral/write-only) that needs Terraform 1.11 or OpenTofu 1.11 and above and is never written to state or to a plan file. Terraform cannot see a write-only value change, so rotating a secret means changing `value_wo_version` as well, conventionally by incrementing it: that is the change Terraform can see, and it is what asks incident.io to rotate. Both are optional once the secret exists, so you can manage a secret's name, description and owning teams while something else looks after its value.
- Add the `incident_secret` data source, which looks up an existing secret by `id` or by `name`, so you can reference one the dashboard owns without pinning an ID that differs between organisations. It reports `version` and `last_four_chars`, which is everything the API will say about a value it never returns. Set exactly one of the two lookup attributes; setting both, or neither, is rejected at plan time.
...
- Add `disabled` to `incident_alert_source` and `incident_alert_source_beta`, which pauses and resumes a heartbeat alert source. Set `disabled = true` to pause monitoring without deleting the source, and `disabled = false` to resume it. Leaving it unset keeps whatever the source is doing, so a pause made in the dashboard survives an unrelated apply. Only heartbeat sources support this; setting it on any other `source_type` is rejected at plan time. On `incident_alert_source_beta` a heartbeat can only be paused once it has received its first ping, which is the v3 API's rule rather than the provider's: a new heartbeat can't be created paused there, so pause it on a later apply.
...
- Add `filter_condition_groups` to `incident_alert_source` and `incident_alert_source_beta`, which drops an incoming event before it becomes an alert when it doesn't match the given conditions. Conditions are evaluated against the event's payload and the source's own expressions, using the same `conditions` shape as `incident_alert_route`'s `condition_groups`. Leave the attribute unset for a source that filters nothing; removing it from a config that had one clears the filters rather than leaving them as they were.
