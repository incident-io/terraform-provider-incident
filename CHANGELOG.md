...
- Add the `incident_incident_template` resource, which manages a standalone incident template - the name and summary autogenerate bindings, severity and its merge strategy, custom fields, incident mode and type, start-in-triage, and workspace - so a template can be defined once and shared across alert routes rather than inlined on each. The template is validated at plan time against the API: an invalid template fails the plan, while a check that can't be reached only warns, so an unavailable validate endpoint never blocks a valid config. Membership teams and incident privacy aren't part of a standalone template; those stay on the alert route.
- Add `incident_config.incident_template` to `incident_alert_route`, which points a route at a standalone `incident_incident_template` instead of defining the template inline under `incident_config.template`. It takes a param binding - a literal template ID or an expression - and is mutually exclusive with the inline `template`, so setting both is rejected at plan time. The reference only applies to routes in the current configuration format (those with `grouping_config`) that create incidents, so it's rejected in the deprecated format and when incident creation is disabled. Set neither and incidents created by the route use the organisation's default template. The resource's registry page now shows the three flows - inline template, standalone reference, and org default - as separate examples.
