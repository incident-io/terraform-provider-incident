...
- `incident_escalation_path_beta` can build a path from a template. Set `template_id` and bind each of the template's params in `param_bindings`, keyed by the param's name, in place of `start` and `sequences`: a templated path takes its nodes, working hours and repeat config from the template, so those four attributes are rejected on one, and `start` and `sequences` are now optional. A path is standalone or templated for life: adding or removing `template_id` replaces the path, while switching from one template to another is an in-place update. The new computed `kind` says which a path is, and the `incident_escalation_path_beta` data source reports `kind`, `template_id` and `param_bindings` too. Plan-time validation covers templated paths, so a binding the template doesn't declare, or a missing one, fails the plan rather than the apply.
