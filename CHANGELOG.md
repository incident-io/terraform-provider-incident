...
- Add `disabled` to `incident_alert_source` and `incident_alert_source_beta`, which pauses and resumes a heartbeat alert source. Set `disabled = true` to pause monitoring without deleting the source, and `disabled = false` to resume it. Only heartbeat sources support this; setting it on any other `source_type` is rejected at plan time. Creating a paused heartbeat is a create followed by an update, because the create payload has no `disabled` field.
